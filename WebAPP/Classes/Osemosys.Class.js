...
export class Osemosys {
    ...
    //asked repeatedly while run() is still in flight; answered on another
    //thread, so it reports the stage the run has actually reached
    //archives run_summary.csv so the next run starts a fresh record
    static clearRunSummary(caserunname) {
        return new Promise((resolve) => {
            $.ajax({
                url: Base.apiUrl() + "clearRunSummary",
                type: 'POST',
                dataType: 'json',
                data: JSON.stringify({ "caserunname": caserunname }),
                contentType: 'application/json; charset=utf-8',
                success: function (result) { resolve(result); },
                error: function () { resolve({ status_code: 'error' }); }
            });
        });
    }

    static progress(casename, caserunname) {
        return new Promise((resolve) => {
            $.ajax({
                url: Base.apiUrl() + "progress",
                type: 'POST',
                dataType: 'json',
                data: JSON.stringify({ "casename": casename, "caserunname": caserunname }),
                contentType: 'application/json; charset=utf-8',
                success: function (result) { resolve(result); },
                error: function () { resolve({ status: 'unknown' }); }
            });
        });
    }

    static run(casename, solver, caserunname, highsOptions) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url:Base.apiUrl() + "run",
                async: true,  
                ...
                data: JSON.stringify({ "casename": casename, "solver": solver, 'caserunname': caserunname,
                                       "highs_options": highsOptions || null }),
                contentType: 'application/json; charset=utf-8',
                // credentials: 'include',
                // xhrFields: { withCredentials: true},
                // crossDomain: true,
